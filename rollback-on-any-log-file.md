...
* バックアップ**N+1番**を別の新規フォルダーに復元し，復元したバックアップN番のフォルダーに，下記のファイルとフォルダーをコピーします。

 * ```Preferences```フォルダー（```backup.xml```が含まれている）
 * ストラクチャファイル（.4DB）

* バックアップ（.4BK）*N番*を復元したバックアップN番のフォルダーにコピーします。

* バックアップログファイル（.4BL）**N-1番**を復元したバックアップN番のフォルダーにコピーします。
